...
def _mock_builder_report(task: dict[str, Any], state: RunState) -> dict[str, Any]:
    ...
    if "force-test-fail" in objective:
        failing = 2
    else:
        failing = 0
...
def _mock_verifier_report(task: dict[str, Any], reviewer_report: dict[str, Any]) -> dict[str, Any]:
    if "force-escalate-disagreement" in task["objective"]:
        approved = not reviewer_report["approved"]
    else:
        approved = reviewer_report["approved"]
    return {
        "agent": "verifier",
        "approved": approved,
...
class OverseerGraph:
    ...
    def compile(self):
        ...
        workflow.add_conditional_edges(
            ...
            {
                "continue": "run_builder",
                "merge": "update_codex",
                "escalate": "update_codex",
            },
        )
    ...
    def run_reviewer(self, state: RunState) -> RunState:
        return {**state, "reviewer_report": _mock_reviewer_report(state["task"], state)}
        ...
        if state["verifier_disputes"] >= self.policy.max_verifier_disputes:
            return {
                **state,
                "decision": "escalate",
                "status": "escalated",
                "escalation_reason": "reviewer/verifier disagreement threshold reached",
            }
        ...
        if state["test_failures_without_progress"] >= self.policy.max_test_failures_without_progress:
            return {
                **state,
                "decision": "escalate",
                "status": "escalated",
                "escalation_reason": "tests failed twice without progress",
            }
        ...
        if cycle_count >= self.policy.max_review_cycles:
            return {
                **state,
                "decision": "escalate",
                "status": "escalated",
                "escalation_reason": "max review cycles reached",
            }
